defmodule InstabotWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, InstabotWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "writes a cookie if remember_me is configured", %{conn: conn, user: user} do
      ...
      assert %{value: signed_token, max_age: max_age, secure: false} = conn.resp_cookies[@remember_me_cookie]
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      InstabotWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, user: user} do
      ...
      assert %{value: new_signed_token, max_age: max_age, secure: false} = conn.resp_cookies[@remember_me_cookie]
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        InstabotWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
